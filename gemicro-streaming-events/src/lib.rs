//! Minimal streaming event types for agent communication.
//!
//! This crate provides the core event types used by gemicro agents to stream
//! updates during execution. It has minimal dependencies (just serde) so it
//! can be used by external projects without pulling in the full gemicro stack.
//!
//! # Core Types
//!
//! - [`AgentUpdate`] - The event type yielded by agent streams
//! - [`FinalResult`] - Strongly-typed completion result
//! - [`ResultMetadata`] - Execution metadata (tokens, duration, extras)
//!
//! # Event Contract
//!
//! All agents must emit exactly one [`EVENT_FINAL_RESULT`] event to signal completion.
//! This is the universal completion signal across all agent implementations.
//!
//! # Example
//!
//! ```
//! use gemicro_streaming_events::{AgentUpdate, ResultMetadata, EVENT_FINAL_RESULT};
//! use serde_json::json;
//!
//! // Create custom events during execution
//! let progress = AgentUpdate::custom("step_completed", "Step 1 done", json!({"step": 1}));
//!
//! // Signal completion with final_result
//! let result = AgentUpdate::final_result(
...
/// The universal completion signal that ALL agents must emit.
///
/// This is the only cross-agent event type. All other event types are
/// agent-specific and defined in their respective crates.
...
/// # Design Philosophy
///
/// - **Extensibility**: New agents can define new event types without modifying core
...
///
/// ```
/// use gemicro_streaming_events::AgentUpdate;
/// use serde_json::json;
///
...
pub struct AgentUpdate {
    ...
    /// (e.g., `as_final_result()`) for ergonomic access.
...
impl AgentUpdate {
    ...
    /// Agents define their own event types without modifying core.
    ///
    /// # Example
    ///
    /// ```
    ...
    ///
    /// ```
    /// use gemicro_streaming_events::{AgentUpdate, ResultMetadata};
    /// use serde_json::json;
    ///
...
mod tests {
    ...
    fn test_accessor_malformed_data() {
        let update = AgentUpdate::custom("final_result", "Test", json!({ "wrong_field": 123 }));
        assert!(update.as_final_result().is_none());
    }
