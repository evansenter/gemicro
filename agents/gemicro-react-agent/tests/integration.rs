...
//! Run with: cargo test -p gemicro-react-agent -- --include-ignored
...
async fn test_react_invalid_config() {
    let config = ReactAgentConfig::default().with_max_iterations(0);
...
async fn test_react_empty_tools_config() {
    let config = ReactAgentConfig::default().with_available_tools(vec![]);
