...
//!
//! ```no_run
//! use gemicro_critique::{CritiqueInput, CritiqueCriteria, CritiqueContext};
//! use serde_json::json;
//!
...
//!
//! ```no_run
//! use gemicro_critique::{CritiqueInput, CritiqueCriteria};
//!
...
impl CritiqueAgent {
    ...
    pub fn new(config: CritiqueConfig) -> Result<Self, AgentError> {
        config.validate()?;
        Ok(Self { config })
    }
...
mod tests {
    ...
    fn test_config_rejects_zero_timeout() {
        let config = CritiqueConfig::default().with_timeout(Duration::ZERO);
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_config_rejects_empty_system() {
        let config = CritiqueConfig::default().with_system_instruction("   ");
        assert!(config.validate().is_err());
    }
    ...
    fn test_agent_creation_validates_config() {
        let invalid = CritiqueConfig::default().with_timeout(Duration::ZERO);
