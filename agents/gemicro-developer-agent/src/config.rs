...
#[derive(Clone, Debug)]
#[non_exhaustive]
pub struct DeveloperAgentConfig {
...
impl Default for DeveloperAgentConfig {
    fn default() -> Self {
        Self {
...
mod tests {
    ...
    fn test_default_config() {
        let config = DeveloperAgentConfig::default();
    ...
    fn test_empty_system_prompt_invalid() {
        let config = DeveloperAgentConfig::default().with_system_prompt("");
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_zero_timeout_invalid() {
        let config = DeveloperAgentConfig::default().with_timeout(Duration::ZERO);
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_zero_max_iterations_invalid() {
        let config = DeveloperAgentConfig::default().with_max_iterations(0);
        assert!(config.validate().is_err());
    }

    #[test]
    fn test_builder_pattern() {
        let config = DeveloperAgentConfig::new()
    ...
    fn test_build_system_prompt_without_di() {
        let config =
            DeveloperAgentConfig::default().with_di_paths(vec![PathBuf::from("/nonexistent/path")]);
