...
//!
//! Run with:
//!   GEMINI_API_KEY=your_key cargo run -p gemicro-developer-agent --example developer
//!
//! Or with a custom query:
//!   GEMINI_API_KEY=your_key cargo run -p gemicro-developer-agent --example developer -- "List files in src/"
//!
//! For full wire-level debugging (API traffic + tool args/results):
//!   LOUD_WIRE=1 GEMINI_API_KEY=your_key cargo run -p gemicro-developer-agent --example developer
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = DeveloperAgentConfig::default().with_max_iterations(50);
